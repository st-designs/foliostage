...
async function coverDataUri(buffer, boxW, boxH, { anchor = 'top', scale = 2, quality = 82, format = 'jpeg' } = {}) {
  ...
  const out = format === 'png'
    ...
    : await image.jpeg(jpegOptions(quality)).toBuffer();
  return `data:image/${format};base64,${out.toString('base64')}`;
}
...
async function widthFitDataUri(buffer, boxW, boxH, { scale = 2, quality = 82, format = 'jpeg' } = {}) {
  ...
  out = out.flatten({ background: '#ffffff' });
  const buf = format === 'png'
    ? await out.png({ compressionLevel: 9 }).toBuffer()
    : await out.jpeg(jpegOptions(quality)).toBuffer();
...
async function composeShowcaseSet({ fullPages = [], tabletPages = [], mobilePages = [], bg, rng, count = 4, exclude = [], family, devices, radius }) {
  ...
  for (const fam of plan) {
    ...
    out.push({
      variant,
      svg: await composeScreensImage(variant, { fullPages, tabletPages, mobilePages, bg, rng, devices, radius }),
    });
  }
  return out;
}
...
// One legible desktop page on the portrait portfolio canvas. Width is privileged:
// long pages continue below the frame instead of being shrunk into a thumbnail.
async function composeSinglePageImage(buf, { bg, brandColor, rng, radius }) {
  const W = 948, H = 1080;
  const R = frameRadius(rng, radius);
  const pad = 100;
  const cardW = W - pad * 2;
  const meta = await sharp(buf).metadata();
  const naturalH = Math.round(meta.height * cardW / meta.width);
  const short = naturalH <= H - pad * 2;
  const cardH = short ? naturalH : Math.max(H - pad, naturalH);
  const x = pad;
  const y = short ? Math.round((H - cardH) / 2) : pad;
  const uri = await widthFitDataUri(buf, cardW, cardH, { scale: 2, format: 'png' });
  const paint = bgPaint(bg, brandColor || LIGHT_BG);
  const id = nid();
  return `${svgOpen(W, H)}
${paint.defs}<rect width="${W}" height="${H}" fill="${paint.fill}"/>
${SHADOW_DEF}<defs>${clipRounded(id, x, y, cardW, cardH, R)}</defs>
<g filter="url(#cardShadow)"><rect x="${x}" y="${y}" width="${cardW}" height="${cardH}" rx="${R}" fill="#fff"/></g>
<image href="${uri}" x="${x}" y="${y}" width="${cardW}" height="${cardH}" clip-path="url(#${id})" preserveAspectRatio="xMidYMin slice"/>
</svg>`;
}

// Present an exact captured section without changing its aspect ratio. Seeded
// placement varies between centered and corner-biased editorial compositions.
async function composeSingleSectionImage(buf, { bg, brandColor, rng, radius }) {
  const W = 1920, H = 1080;
  const R = frameRadius(rng, radius);
  const meta = await sharp(buf).metadata();
  const maxW = Math.round(W * range(rng, 0.72, 0.88));
  const maxH = Math.round(H * range(rng, 0.68, 0.84));
  let cardW = maxW, cardH = Math.round(maxW * meta.height / meta.width);
  if (cardH > maxH) { cardH = maxH; cardW = Math.round(maxH * meta.width / meta.height); }
  const inset = Math.round(range(rng, 90, 150));
  const placement = pick(rng, ['center', 'center', 'top-left', 'top-right', 'bottom-left', 'bottom-right']);
  const x = placement === 'center' ? Math.round((W - cardW) / 2) : placement.includes('right') ? W - inset - cardW : inset;
  const y = placement === 'center' ? Math.round((H - cardH) / 2) : placement.includes('bottom') ? H - inset - cardH : inset;
  const uri = await coverDataUri(buf, cardW, cardH, { scale: 2, format: 'png' });
  const paint = bgPaint(bg, brandColor || LIGHT_BG);
  const id = nid();
  return `${svgOpen(W, H)}
${paint.defs}<rect width="${W}" height="${H}" fill="${paint.fill}"/>
${SHADOW_DEF}<defs>${clipRounded(id, x, y, cardW, cardH, R)}</defs>
<g filter="url(#cardShadow)"><rect x="${x}" y="${y}" width="${cardW}" height="${cardH}" rx="${R}" fill="#fff"/></g>
<image href="${uri}" x="${x}" y="${y}" width="${cardW}" height="${cardH}" clip-path="url(#${id})" preserveAspectRatio="xMidYMid slice"/>
</svg>`;
}

module.exports = { composeAll, composeOne, composeLogoSet, composeMockupSet, composeMockupImage, composeShowcaseSet, composeFeaturedSet, composeShotImage, composeSinglePageImage, composeSingleSectionImage, composeScreensImage, MOCKUP_VARIANTS, SHOWCASE_LIGHT, SHOWCASE_DENSE, bgCss };
