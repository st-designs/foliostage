...
function resetCaptureCancel() { CAPTURE_CANCELLED = false; }
function requestCaptureCancel() {
  CAPTURE_CANCELLED = true;
  for (const browser of ACTIVE_BROWSERS) browser.close().catch(() => {});
}
...
// trigger macOS App Management prompts, and FolioStage — Portfolio Generator does not need
...
async function launchBrowser(log = () => {}) {
  if (CAPTURE_CANCELLED) throw new Error('Capture cancelled');
  const launchArgs = { args: ['--no-sandbox', '--disable-dev-shm-usage'] };
  try {
    const b = await chromium.launch(launchArgs);
    if (CAPTURE_CANCELLED) { await b.close().catch(() => {}); throw new Error('Capture cancelled'); }
    ACTIVE_BROWSERS.add(b);
    b.on('disconnected', () => ACTIVE_BROWSERS.delete(b));
    log('Using FolioStage — Portfolio Generator capture browser');
    return b;
  } catch {}
...
// Full-page screenshot at the browser's real device scale. Very tall pages
// use viewport stitching so they keep 2x detail without hitting texture limits.
async function fullPageShot(p) {
  ...
  if (docH <= 12000) {
    try { return await p.screenshot({ type: 'png', fullPage: true, scale: 'device' }); } catch {}
  }
  return fullPageShotStitched(p);
}
...
async function fullPageShotStitched(p, log = () => {}) {
  const vp = p.viewportSize();
  const docH = await p.evaluate(() => Math.max(document.body.scrollHeight, document.documentElement.scrollHeight));
  const H = Math.min(docH, 15000), W = vp.width, S = SCALE;
  ...
  try {
    for (let y = 0; y < H; y += vp.height) {
      ...
      await fixVideos(p);
      const shot = await p.screenshot({ type: 'png', scale: 'device' });
      ...
      const img = (cropTop > 0 || tileH < vp.height)
        ? await sharp(shot).extract({ left: 0, top: cropTop * S, width: W * S, height: tileH * S }).toBuffer()
        : shot;
      comps.push({ input: img, left: 0, top: y * S });
      if (first) {
        first = false;
  ...
  return sharp({ create: { width: W * S, height: H * S, channels: 3, background: '#ffffff' } })
...
async function autoScroll(page) {
  ...
  try {
    await page.evaluate(async (pause) => {
      ...
      const max = Math.min(Math.max(document.body.scrollHeight, document.documentElement.scrollHeight), 40000);
...
async function stabilize(page) {
  ...
  try {
    await page.evaluate(async (IMG_WAIT) => {
      ...
      // Do not render Chromium's broken-image badge into an otherwise clean
      // portfolio. Keep the box in flow so hiding it cannot disturb layout.
      document.images.forEach((img) => {
        if (img.complete && img.naturalWidth === 0) img.style.setProperty('visibility', 'hidden', 'important');
      });
    }, P.imgWait);
...
function hideFloatersInPage(loop) {
  ...
  const pass = () => {
    ...
    for (const el of document.querySelectorAll('body *')) {
      ...
      const semanticFloater = /(chat|support|help|whatsapp|messenger|contact|launcher|bubble|newsletter|subscribe|exit.intent|back.?to.?top|scroll.?top|arrow.?up)/.test(words);
      const smallCornerControl = corner && z >= 5 && r.width <= 120 && r.height <= 120;
      const bottomCta = r.bottom > vh * 0.86 && r.width > vw * 0.45 && r.height < vh * 0.35
        && !!el.querySelector('button, [role="button"], a[href]');
      if (((semanticFloater || smallCornerControl) && corner || bottomCta) && z >= 5 && r.width <= vw * 1.01 && r.height < vh * 0.45) {
...
async function settle(page, log = () => {}) {
  ...
  await fixVideos(page);
  await stabilize(page);
  await forceRevealHidden(page);
  await refreshCaptureSurface(page);
...
async function pickSectionBoxes(page, excludeTops = [], limit = 1) {
  return page.evaluate(({ excludeTops, limit }) => {
    const lum = (c) => {
    ...
    for (const el of document.querySelectorAll('section, [class*="section" i], main > div, [class*="row" i]')) {
      const r = el.getBoundingClientRect();
      const top = r.top + scrollY, left = r.left + scrollX, h = r.height, w = r.width;
      if (h < 260 || h > 1600 || w < vw * 0.55 || top < vh * 0.45) continue;
      ...
      if (score < 1) continue;
      cands.push({ top, left: Math.max(0, left), w: Math.min(w, vw), h: Math.min(h, 1500), score });
    }
    cands.sort((a, b) => b.score - a.score || a.top - b.top);
    const chosen = [];
    for (const cand of cands) {
      const overlaps = chosen.some((item) => {
        const intersection = Math.max(0, Math.min(item.top + item.h, cand.top + cand.h) - Math.max(item.top, cand.top));
        return intersection / Math.min(item.h, cand.h) > 0.55;
      });
...
async function pickSectionBox(page, excludeTops = []) {
  const boxes = await pickSectionBoxes(page, excludeTops, 1);
  return boxes[0] || null;
}

async function hideSectionViewportChrome(page) {
  await page.evaluate(() => {
    const vw = innerWidth, vh = innerHeight;
    for (const el of document.querySelectorAll('body *')) {
      if (!(el instanceof HTMLElement)) continue;
      ...
      const r = el.getBoundingClientRect();
      const fixed = cs.position === 'fixed';
      const topStickyChrome = cs.position === 'sticky' && r.top <= 24 && r.height < vh * 0.35;
      const bottomChrome = (fixed || cs.position === 'sticky') && r.bottom >= vh - 8 && r.height < vh * 0.4;
      if ((fixed || topStickyChrome || bottomChrome) && r.width > 0 && r.width <= vw * 1.02) {
        el.style.setProperty('visibility', 'hidden', 'important');
      }
    }
  }).catch(() => {});
...
async function shootSection(page, box) {
  ...
  await hideSectionViewportChrome(page);
  await page.waitForTimeout(250);
  // fullPage makes the clip document-relative: exact section, nothing cut off
  return page.screenshot({
    type: 'png', scale: 'device', fullPage: true,
    clip: { x: Math.round(box.left || 0), y: Math.round(box.top), width: Math.round(box.w || page.viewportSize().width), height: Math.round(box.h) },
  });
}
...
async function captureHero(url, log = () => {}, viewports) {
  ...
  return withPage(url, async (page) => {
    ...
    await fixVideos(page);
    const heroTall = await stableShot(page, () => page.screenshot({ type: 'png', scale: 'device' }), log);
...
async function captureFullPages(urls, log = () => {}, viewports) {
  const VP = viewportsFrom(viewports);
  const browser = await launchBrowser(log);
  const out = [];
  try {
    ...
    for (const url of urls) {
      try {
        ...
        await settle(page, log);
        out.push({ url, buffer: await fullPageShotChecked(page, log, url) });
      } catch (error) { log(`  desktop skipped: ${url} (${error.message.split('\n')[0]})`); }
    }
    await ctx.close();
...
async function captureIdentity(url, log = () => {}, viewports) {
  return withPage(url, async (page) => {
    const logo = await extractLogo(page, url, log);
    const mark = await extractMark(page, url, log);
    const colors = await page.evaluate(() => {
      const values = [];
      const theme = document.querySelector('meta[name="theme-color"]')?.getAttribute('content');
      if (theme) values.push(theme);
      for (const selector of ['header', 'nav', 'button', 'footer']) {
        const el = document.querySelector(selector);
        if (el) values.push(getComputedStyle(el).backgroundColor, getComputedStyle(el).color);
      }
      return values;
    }).catch(() => []);
    return {
      logo, mark,
      brandColor: pickBrandColor(colors.map(parseCssColor).filter(Boolean)),
      logoIsLight: logo ? await logoIsLight(logo) : false,
    };
  }, { log, viewports });
}

async function captureSections(url, excludeTops = [], limit = 1, log = () => {}, viewports) {
  return withPage(url, async (page) => {
    const boxes = await pickSectionBoxes(page, excludeTops, limit);
    const sections = [];
    for (const box of boxes) sections.push({ url, buffer: await shootSection(page, box), top: box.top, score: box.score });
    return sections;
  }, { log, viewports });
}

async function captureSection(url, excludeTops = [], log = () => {}, viewports) {
  return (await captureSections(url, excludeTops, 1, log, viewports))[0] || null;
}
...
async function capture(targetUrl, opts = {}) {
  ...
  const browser = await launchBrowser(log);
  const result = { screenshots: {}, sections: [], fullPages: [], mobilePages: [], meta: { viewports: VP, diagnostics: [] } };
  ...
  try {
    ...
    await page.waitForTimeout(600);
    await refreshCaptureSurface(page, { loop: true });
    result.screenshots.heroTall = await stableShot(page, () => page.screenshot({ type: 'png', scale: 'device' }), log);
    ...
    // Keep several strong, non-overlapping sections. Single Sections uses
    // these exact DOM bounds rather than guessing crops from a full page.
    try {
      const boxes = await pickSectionBoxes(page, [], 4);
      for (const box of boxes) {
        result.sections.push({ url: targetUrl, buffer: await shootSection(page, box), top: box.top, score: box.score });
      }
    } catch (e) { log(`  homepage section skipped (${e.message.split('\n')[0]})`); }
    ...
    for (const u of pageUrls) {
      try {
        ...
        try {
          const boxes = await pickSectionBoxes(page, [], 3);
          for (const box of boxes) result.sections.push({ url: u, buffer: await shootSection(page, box), top: box.top, score: box.score });
        } catch (e) { log(`  section skipped (${e.message.split('\n')[0]})`); }
    ...
    try {
      ...
      if (box) {
        ...
        if (!result.sections.length) result.sections.push({ url: targetUrl, buffer: result.screenshots.section, top: box.top, score: box.score });
      } else {
    ...
    if (!result.sections.length) result.sections.push({ url: targetUrl, buffer: result.screenshots.section || result.screenshots.hero, top: 0, score: 0 });
...
module.exports = { capture, captureHero, captureFullPage, captureFullPages, captureIdentity, captureSection, captureSections, captureMobiles, captureTablets, captureShots, recordScrollVideo, setPatience, setOverlayCleanup, setExtraWait, setStitchMode, resetCaptureCancel, requestCaptureCancel };
